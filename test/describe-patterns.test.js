...
// Fix round 3, IMPORTANT 2: canonicalPlane picked its sign-defining component with a
// bare `>` comparison, so a mirror plane whose normal has two components equal in
// magnitude -- which is exactly what happens at a 45deg rotation, since the
// x=const/y=const planes' normals become (+-cos45, +-cos45, 0) -- let float noise
// between two independently-computed proposing pairs decide the tie differently,
// canonicalizing the SAME plane to opposite-sign normals that then bucket separately
// and each fail the SYMMETRY_EVIDENCE_MIN gate alone. NOT reproducible with this
// file's own hand-rotated fixtures, though: every hole here is generated by the same
// shared `rot()` call, so every proposing pair's normal is computed via bit-identical
// trig arithmetic and never disagrees with itself. The bug needs INDEPENDENT float
// noise across proposers -- real tessellation noise from an actual meshed part -- so
// its regression guard lives in test/describe.test.js ("a real meshed 2x2-hole plate
// rotated exactly 45deg about Z still reports both mirror planes"), which builds the
// plate through the Manifold kernel and runs the full describeMesh() pipeline instead
// of calling detectPatterns on synthetic positions.
