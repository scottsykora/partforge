...
test("explicit shading hint wins in both directions", () => {
  const rings = resolveLoftRings([{ sides: 12, radius: 20, z: 0 }, { sides: 12, radius: 20, z: 10 }]);
  expect(loftShadingPolicy(rings, { shading: "smooth" })).toBe(SMOOTH);
  expect(loftShadingPolicy(rings, { shading: "faceted" })).toBe(FACETED);
  const many = resolveLoftRings([{ sides: 64, radius: 20, z: 0 }, { sides: 64, radius: 20, z: 10 }]);
...
test("ruled:false (OCCT smooth blend) implies smooth shading intent", () => {
  const rings = resolveLoftRings([{ sides: 6, radius: 20, z: 0 }, { sides: 6, radius: 20, z: 10 }]);
...
test("inference: low side counts are facets, high counts approximate smooth", () => {
  const few = resolveLoftRings([{ sides: 12, radius: 20, z: 0 }, { sides: 12, radius: 20, z: 10 }]);
  expect(loftShadingPolicy(few, {})).toBe(FACETED);
  const many = resolveLoftRings([{ sides: SMOOTH_SIDES_MIN, radius: 20, z: 0 }, { sides: SMOOTH_SIDES_MIN, radius: 20, z: 10 }]);
...
test("inference reads explicit polygon rings by point count", () => {
  ...
  expect(loftShadingPolicy(resolveLoftRings([{ polygon: poly(8), z: 0 }, { polygon: poly(8), z: 5 }]), {})).toBe(FACETED);
  expect(loftShadingPolicy(resolveLoftRings([{ polygon: poly(48), z: 0 }, { polygon: poly(48), z: 5 }]), {})).toBe(SMOOTH);
});
