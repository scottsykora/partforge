...
test("volume() returns the solid volume", () => {
  expect(k.box({ min: [0, 0, 0], max: [10, 10, 10] }).volume()).toBeCloseTo(1000, 0);
});

test("fillet removes a little volume and still meshes", () => {
  const sharp = k.box({ min: [0, 0, 0], max: [20, 20, 20] }).volume();          // 8000
  const filleted = k.box({ min: [0, 0, 0], max: [20, 20, 20] }).fillet({ r: 2, edges: { dir: "Z" } });
...
test("selecting all edges removes more than selecting only vertical edges", () => {
  const vertical = k.box({ min: [0, 0, 0], max: [20, 20, 20] }).fillet({ r: 2, edges: { dir: "Z" } }).volume();
  const all = k.box({ min: [0, 0, 0], max: [20, 20, 20] }).fillet(2).volume();
  expect(all).toBeLessThan(vertical);
});

test("chamfer removes volume", () => {
  expect(k.box({ min: [0, 0, 0], max: [20, 20, 20] }).chamfer(2).volume()).toBeLessThan(8000);
});

test("toIndexedMesh returns positions and indices", () => {
  const m = k.box({ min: [0, 0, 0], max: [5, 5, 5] }).toIndexedMesh();
...
test("an out-of-range fillet is skipped, not fatal — the shape survives", () => {
  ...
  const v = k.box({ min: [0, 0, 0], max: [10, 10, 10] }).fillet(50).volume();
...
test("an out-of-range chamfer is reduced to the largest valid value, not fatal", () => {
  ...
  const v = k.box({ min: [0, 0, 0], max: [10, 10, 10] }).chamfer(50).volume();
...
test("chamfer validates the result: an oversize request is reduced to the largest valid, not skipped or broken", () => {
  ...
  const filleted = k.box({ min: [0, 0, 0], max: [40, 30, 16] }).fillet({ r: 3, edges: { dir: "Z" } }).volume();
  const huge = k.box({ min: [0, 0, 0], max: [40, 30, 16] }).fillet({ r: 3, edges: { dir: "Z" } }).chamfer({ d: 20, edges: { inPlane: "XY", at: 0 } });
...
test("a chamfer that fits is applied at the requested value", () => {
  const filleted = k.box({ min: [0, 0, 0], max: [40, 30, 16] }).fillet({ r: 3, edges: { dir: "Z" } }).volume();
  const v = k.box({ min: [0, 0, 0], max: [40, 30, 16] }).fillet({ r: 3, edges: { dir: "Z" } }).chamfer({ d: 2, edges: { inPlane: "XY", at: 0 } }).volume();
