...
const TOOLS = [
  ...
  { tool: "line", icon: LINE_ICON, label: "Line", hint: "drag endpoint to endpoint · shift = 45° snap" },
  ...
  { tool: "eraser", icon: ERASER_ICON, label: "Eraser", hint: "scrub to erase — shapes remember themselves" },
];
...
export function attachSketchToolbar(mode, { stage, tooltip, send = "viewbar" } = {}) {
  ...
  const allButtons = [...toolButtons, ...swatchButtons, undoButton, clearButton, sendButton].filter(Boolean);
