...
- **Detected:** The exact kernel now refuses the common cylindrical form of this contact up front — a swept or curved face lying exactly on a cylindrical face fails the boolean immediately with `<op> between exactly-touching surfaces: … (radius <r>)` and the fix menu below, instead of grinding. The guard covers curved-face-on-cylinder contact only; exact contact between two swept faces, or with non-cylindrical faces, can still hang — the rule below applies everywhere regardless.
