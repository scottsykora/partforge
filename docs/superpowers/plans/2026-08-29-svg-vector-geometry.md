...
- **The worker graph stays DOM-free.** Every file under `src/framework/geometry/` and anything reachable from `jobs.js` must be free of `three`, `node:`, and the DOM. `test/worker-layering.test.js` greps module SOURCE for the words `document`, `window`, `localStorage`, `sessionStorage`, `HTMLElement`, `customElements`. **`src/framework/ingest/` and `src/ingest.js` are the sole exception and must never be imported from the worker graph.**
- **The word `document` is a trap in this feature specifically**, because the format's own instances are called documents. `worker-layering.test.js:33` strips comments before it greps, so prose may say `document` freely — but **string literals are not stripped**, so an error message containing a standalone `document` will fail the check for any worker-graph module. `validateVectorDocument` and `VectorDocument` are safe (no word boundary before `document`). In worker-graph files (`vector-format.js`, `arc-fit.js`, `stroke-outline.js`, `svg2d.js`, `svgs.js`) say **"file"** in user-facing message text. `svg-ingest.js` is outside the worker graph and unaffected.
...
no default to fall back on) (error). Both judge the argument values the probe
