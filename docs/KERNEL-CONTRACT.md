...
| `vector2d(name, {width\|height\|fit, align?, valign?})` | Ingested vector artwork → `Shape2D`. `name` = a declared name in the part's `vectors` field (ingested `partforge-vector` JSON, not raw `.svg`). Exactly one of `width`/`height`/`fit` sizes the artwork in millimetres by uniform scale (`fit` = larger extent); `align`/`valign` position it, same as `text2d`. Conformance: **both backends** (it lowers to `shape2d` + `union`, exactly like `text2d`). Parity: **identical across backends by construction** — the regions are curve-native (arcs/cubics) from ingest, so there is no sampling step for the two backends to diverge over. |
